(function() {
	...
	var GraphicsEngine = my.Class({
		...
		init: function(urls, callback) {
			...
			sim.controls = new THREE.CameraControls( this.camera );
		...
		addBeam: function(width, length) {
			...
			var lightBeam = new THREE.PlaneGeometry(length, width, width, 100);
			var lightBeamObject = new THREE.Mesh(lightBeam, lightMaterial);

			var lightBeam2 = new THREE.PlaneGeometry(length, width, width, 100);
			var lightBeamObject2 = new THREE.Mesh(lightBeam2, lightMaterial);
			lightBeamObject2.rotation.x = Math.PI/3;

			var lightBeam3 = new THREE.PlaneGeometry(length, width, width, 100);
			var lightBeamObject3 = new THREE.Mesh(lightBeam3, lightMaterial);
			lightBeamObject3.rotation.x = 2 * Math.PI/3;

			var group = new THREE.Object3D();
			group.add( lightBeamObject );
			group.add( lightBeamObject2 );
			group.add( lightBeamObject3 );
			...
			var vector = new THREE.Vector3( -3, 6, -3 );
			...
			newPosition = d.clone().multiply(new THREE.Vector3(length/2, length/2, length/2)).add(vector);



			group.rotation.set(0, camRot.y - Math.PI/2, -camRot.x);
		...
		addGround: function() {
			...
			var floorTexture = new THREE.ImageUtils.loadTexture( 'res/textures/grass.png' );
...
// this.renderer.shadowMapHeight = 1024;
		...
		addSkyDome: function() {
			...
			loader.load( 'res/textures/sky.png', function ( image ) {
