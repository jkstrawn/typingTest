(function() {
	...
	var CastleSim = my.Class({
		...
		createMob: function(mobData) {
			model = this.graphics.createMobModel();
			model.position.set( mobData.position.x, mobData.position.y + 10, mobData.position.z );
			mobData.model = model;
			this.graphics.addModel(model);
			this.graphics.createCircleAroundMob(mobData.position.x, mobData.position.y, mobData.position.z);
			this.shapes.push(shape);
		},
		...
		click: function( event ) {
			...
			//left click
				...
				var box = this.graphics.addBeam(3, 15);
	        	var bullet = new SIM.Projectile(box, d);


				bullet.position = box.position.clone();
