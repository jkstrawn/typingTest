...
AStar.prototype.Path = function() {
	var	_start = this.Node(null, {x: this.start.gridX, y: this.start.gridY});
	var _goal = this.Node(null, {x: this.goal.gridX, y: this.goal.gridY});
	...
	while(length = Open.length) {
		...
		if(_node.value === _goal.value) {
			...
			do {
				result.push({x: _path.x, y: _path.y});
